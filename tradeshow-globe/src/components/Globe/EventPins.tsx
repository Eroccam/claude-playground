...
export function EventPins() {
  const { events, selectedRegion, openCardIds, selectEventFromPin, isSearchMode, isCalendarMode, highlightedEvents } = useGlobe();
  const pinnedEvents = useMemo(() => events.filter((event) => event.hasPin !== false), [events]);
  const highlightedEventIds = useMemo(() => new Set(highlightedEvents.map((event) => event.id)), [highlightedEvents]);
  const isHighlightMode = isSearchMode || isCalendarMode;
    ...
    <group>
      {pinnedEvents.map((event) => (
        <EventPin
          key={event.id}
          event={event}
          isSelected={openCardIds.includes(event.id)}
          isInSelectedRegion={event.region === selectedRegion}
          isSearchMode={isHighlightMode}
          isSearchMatch={highlightedEventIds.has(event.id)}
          offset={offsets.get(event.id) ?? new THREE.Vector3()}
          onClick={() => selectEventFromPin(event.id, event.region)}
        />
      ))}
    </group>
  );
