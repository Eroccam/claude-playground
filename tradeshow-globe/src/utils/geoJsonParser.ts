...
/** Earcut base fill, pre-merged by region — one position + normals buffer per region. */
export interface ParsedRegionFills {
  US: Float32Array;
  EMEA: Float32Array;
  APAC: Float32Array;
  /** Per-vertex sphere normals (normalized positions), one buffer per region. */
  normals: Record<Region, Float32Array>;
}
...
// ---------------------------------------------------------------------------
// Normals for sphere-projected vertices: normal == normalized position.
// ---------------------------------------------------------------------------
function buildNormals(positions: Float32Array): Float32Array {
...
export function parseBaseFill(geojson: GeoJSON, radius: number): ParsedRegionFills {
  ...
  const posUS   = new Float32Array(buckets.US);
  const posEMEA = new Float32Array(buckets.EMEA);
  const posAPAC = new Float32Array(buckets.APAC);
  ...
  return {
    ...
    APAC: posAPAC,
    normals: {
      US:   buildNormals(posUS),
      EMEA: buildNormals(posEMEA),
      APAC: buildNormals(posAPAC),
    },
  };
}
